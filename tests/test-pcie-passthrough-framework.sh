...
# Test framework for validating PCIe GPU passthrough in HPC compute images
#
...
show_help() {
    cat << EOF
PCIe Passthrough Test Framework - GPU Passthrough Validation
...
COMMANDS:
    ...
    deploy-ansible    Deploy GPU passthrough via Ansible (assumes cluster is running)
    run-tests         Run GPU passthrough tests on deployed cluster
...
EXAMPLES:
    ...
    $0 deploy-ansible         # Deploy GPU passthrough
    $0 run-tests              # Run tests (can repeat)
    $0 list-tests             # Show all available tests
    $0 run-test check-gpu-visibility.sh  # Run specific test
...
CONFIGURATION:
    ...
    Log Directory: logs/pcie-passthrough-test-run-*
...
export TEST_NAME="pcie-passthrough"
...
# Test configuration
TEST_CONFIG="$PROJECT_ROOT/tests/test-infra/configs/test-pcie-passthrough-minimal.yaml"
TEST_SCRIPTS_DIR="$PROJECT_ROOT/tests/suites/gpu-validation"
TARGET_VM_PATTERN="compute"
MASTER_TEST_SCRIPT="run-all-tests.sh"
...
run_tests() {
    log "Running PCIe passthrough tests on deployed cluster..."
...
run_full_test() {
    ...
    log "Starting PCIe Passthrough Test Framework (GPU Passthrough Validation)"
