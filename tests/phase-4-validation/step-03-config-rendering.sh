#!/bin/bash
#
# Phase 4 Validation: Step 3 - Configuration Template Rendering and VirtIO-FS
#
...
show_step_help() {
  cat << 'EOF'
Phase 4 Validation - Step 03: Configuration Template Rendering and VirtIO-FS

Usage: ./step-03-config-rendering.sh [OPTIONS]
...
main() {
  log_step_title "03" "Configuration Template Rendering and VirtIO-FS"
  ...
  if is_step_completed "step-03-config-rendering"; then
    log_warning "Step 03 already completed at $(get_step_completion_time 'step-03-config-rendering')"
    return 0
  fi

  init_state
  local step_dir="$VALIDATION_ROOT/03-config-rendering"
  create_step_dir "$step_dir"

  cd "$PROJECT_ROOT"

  # 3.1: Test ai-how render command
  log_info "3.1: Testing ai-how render command with variable expansion..."
  ...
  # 3.2: Test make config-render and config-validate targets
  log_info "3.2: Testing make config-render and config-validate targets..."
  ...
  # 3.3: Verify VirtIO-FS mount configuration
  log_info "3.3: Verifying VirtIO-FS mount configuration in cluster config..."
  ...
  # 3.4: Test cluster state directory management
  log_info "3.4: Testing cluster state directory management..."
  ...
  # 3.5: Test template variable detection
  log_info "3.5: Testing template variable detection..."
...
=== Step 03: Configuration Template Rendering and VirtIO-FS ===
Timestamp: $(date)
...
EOF
  ...
  mark_step_completed "step-03-config-rendering"
  log_success "Step 03 PASSED: Configuration template rendering and VirtIO-FS validation complete"
