...
show_step_help() {
  cat << 'EOF'
Phase 4 Validation - Step 04: Runtime Deployment

Usage: ./step-04-runtime-deployment.sh [OPTIONS]
...
main() {
  log_step_title "04" "Runtime Playbook Deployment"
  ...
  if is_step_completed "step-04-runtime-deployment"; then
    log_warning "Step 04 already completed at $(get_step_completion_time 'step-04-runtime-deployment')"
    return 0
  fi

  init_state
  local step_dir="$VALIDATION_ROOT/04-runtime-playbook"
  create_step_dir "$step_dir"

  cd "$PROJECT_ROOT"

  log_info "4.1: Checking playbook syntax..."
  ...
  log_info "4.2: Generating Ansible inventory..."
  ...
  log_info "4.3: Validating cluster configuration..."
  local rendered_config="$VALIDATION_ROOT/03-config-rendering/rendered-config.yaml"
  ...
  log_info "4.4: Starting cluster VMs (2-5 minutes)..."
  ...
  log_info "4.5: Waiting for VMs to initialize (30 seconds)..."
  sleep 30
  ...
  log_info "4.6: Deploying runtime configuration (10-20 minutes)..."
  ...
  log_info "4.7: Analyzing deployment results..."
...
EOF
  ...
  mark_step_completed "step-04-runtime-deployment"
  log_success "Step 04 PASSED: Runtime deployment successful"
