...
check_system_requirements() {
    log_info "Checking system requirements for GPU GRES tests..."
    ...
    if [ "${TEST_MODE:-local}" = "remote" ]; then
        ...
        log_info "Will execute tests via SSH on controller"
...
cleanup_test_environment() {
    ...
    # No specific cleanup needed for GPU GRES tests
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "  TEST_MODE       Set to 'remote' for cluster mode (default: 'local')"
            echo ""
            echo "Test Scripts:"
            for script in "${TEST_SCRIPTS[@]}"; do
                echo "  - $script"
