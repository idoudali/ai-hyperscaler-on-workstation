#!/bin/bash
# Test Suite 2: SLURM Container Execution Test
set -euo pipefail
...
test_srun_container() {
  ...
  if $SSH_CMD "${TEST_CONTROLLER}" "srun --container=$REGISTRY_PATH/$test_image python3 --version >/dev/null 2>&1"; then
    ((TESTS_PASSED++)); echo -e "${GREEN}[PASS]${NC} SLURM container execution works"; return 0
  else
    ((TESTS_FAILED++)); echo -e "${RED}[FAIL]${NC} SLURM container execution failed"; return 1
  fi
}

main() {
  echo "  SLURM Container Execution Test"
  [[ -z "${TEST_CONTROLLER:-}" ]] && echo -e "${RED}ERROR: TEST_CONTROLLER not set${NC}" && exit 1
  test_srun_container || true
