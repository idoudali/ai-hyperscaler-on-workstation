...
test_pytorch_distributed_import() {
  ...
  log_command "$CONTAINER_RUNTIME exec $CONTAINER_IMAGE python3 -c 'import torch.distributed as dist; print(\"Distributed module available\")'"

  local dist_test
  dist_test=$($CONTAINER_RUNTIME exec "$CONTAINER_IMAGE" python3 -c 'import torch.distributed as dist; print("Distributed module available")' 2>&1 || echo "IMPORT_FAILED")
...
EOFPY
  ...
  backends_result=$($CONTAINER_RUNTIME exec "$CONTAINER_IMAGE" python3 "$test_script" 2>&1 || echo "FAILED")
...
EOFPY
  ...
  log_command "$CONTAINER_RUNTIME exec $CONTAINER_IMAGE python3 $test_script"
  local nccl_result
  nccl_result=$($CONTAINER_RUNTIME exec "$CONTAINER_IMAGE" python3 "$test_script" 2>&1 || echo "FAILED")
...
EOFPY
  ...
  local env_result
  env_result=$(MASTER_ADDR=localhost MASTER_PORT=29500 WORLD_SIZE=1 RANK=0 LOCAL_RANK=0 $CONTAINER_RUNTIME exec "$CONTAINER_IMAGE" python3 "$test_script" 2>&1 || echo "FAILED")
...
EOFPY
  ...
  local pg_result
  pg_result=$($CONTAINER_RUNTIME exec "$CONTAINER_IMAGE" python3 "$test_script" 2>&1 || echo "FAILED")
...
EOFPY
  ...
  local ddp_result
  ddp_result=$($CONTAINER_RUNTIME exec "$CONTAINER_IMAGE" python3 "$test_script" 2>&1 || echo "FAILED")
...
EOFPY
  ...
  local coord_result
  coord_result=$($CONTAINER_RUNTIME exec "$CONTAINER_IMAGE" mpirun -np 2 python3 "$test_script" 2>&1 || echo "FAILED")
...
EOFPY
  ...
  sampler_result=$($CONTAINER_RUNTIME exec "$CONTAINER_IMAGE" python3 "$test_script" 2>&1 || echo "FAILED")
...
main() {
  ...
  test_pytorch_distributed_import || ((TESTS_FAILED+=1))
  test_distributed_backends || ((TESTS_FAILED+=1))
  test_nccl_availability || ((TESTS_FAILED+=1))
  test_environment_variables || ((TESTS_FAILED+=1))
  test_process_group_initialization || ((TESTS_FAILED+=1))
  test_distributed_data_parallel || ((TESTS_FAILED+=1))
  test_multi_process_coordination || ((TESTS_FAILED+=1))
  test_distributed_sampler || ((TESTS_FAILED+=1))
