...
show_help() {
    cat << EOF
SLURM Controller Test Framework - Task 010 Validation
...
COMMANDS:
    ...
    deploy-ansible    Deploy SLURM controller via Ansible (assumes cluster is running)
    run-tests         Run SLURM controller tests on deployed cluster
...
EXAMPLES:
    ...
    $0 deploy-ansible         # Deploy SLURM controller
...
CONFIGURATION:
    ...
    Log Directory: logs/slurm-controller-test-run-*
...
run_tests() {
    log "Running SLURM controller tests on deployed cluster..."
...
main() {
    ...
    case "$COMMAND" in
        ...
        "list-tests")
            ...
            list_tests_in_directory "$TEST_SCRIPTS_DIR" "SLURM Controller Test Scripts" "$0"
            exit 0
            ;;
        "run-test")
            # Run specific test
