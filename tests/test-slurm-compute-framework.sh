#!/bin/bash
#
# SLURM Compute Node Test Framework
# Task 022 - SLURM Compute Node Installation and Functionality Validation
# Test framework for validating SLURM compute node installation in HPC compute images
#
...
FRAMEWORK_NAME="SLURM Compute Node Test Framework"
...
export TEST_NAME="slurm-compute"

# shellcheck source=./test-infra/utils/test-framework-utils.sh
source "$UTILS_DIR/test-framework-utils.sh"

# Test configuration
TEST_CONFIG="$PROJECT_ROOT/tests/test-infra/configs/test-slurm-compute.yaml"
TEST_SCRIPTS_DIR="$PROJECT_ROOT/tests/suites/slurm-compute"
TARGET_VM_PATTERN="compute"
MASTER_TEST_SCRIPT="run-slurm-compute-tests.sh"

# Initialize logging
TIMESTAMP=$(date '+%Y-%m-%d_%H-%M-%S')
init_logging "$TIMESTAMP" "logs" "slurm-compute"

# Individual command functions
# Note: start_cluster, generate_ansible_inventory, deploy_ansible_playbook are provided by utilities

deploy_ansible() {
    log "Deploying SLURM compute configuration to cluster: $TEST_NAME"

    local playbook="$PROJECT_ROOT/ansible/playbooks/playbook-slurm-compute-runtime-config.yml"
    ...
    log "  - Node registration will be TESTED"
    log ""
    ...
    if ! deploy_ansible_playbook "$TEST_CONFIG" "$playbook" "compute" "$LOG_DIR"; then
        log_error "Failed to deploy SLURM compute configuration"
        return 1
    fi

    log_success "SLURM compute configuration deployed successfully"
    return 0
}
...
list_tests() {
    ...
    echo "Example: $0 run-test check-slurm-installation.sh"
}
...
main() {
    ...
    log "Starting SLURM Compute Node Test Framework (Task 022 Validation)"
    log "Configuration: $TEST_CONFIG"
    ...
    if run_test_framework "$TEST_CONFIG" "$TEST_SCRIPTS_DIR" "$TARGET_VM_PATTERN" "$MASTER_TEST_SCRIPT"; then
        ...
        echo ""
        echo -e "${GREEN}==========================================${NC}"
        echo -e "${GREEN}  SLURM Compute Node Tests PASSED${NC}"
        echo -e "${GREEN}==========================================${NC}"
        echo -e "${GREEN}Total execution time: ${duration}s${NC}"
        echo ""

        log "SLURM Compute Node test framework completed successfully (${duration}s)"
        exit 0
    else
        local end_time
        end_time=$(date +%s)
        ...
        echo ""
        echo -e "${RED}==========================================${NC}"
        echo -e "${RED}  SLURM Compute Node Tests FAILED${NC}"
        echo -e "${RED}==========================================${NC}"
        echo -e "${RED}Total execution time: ${duration}s${NC}"
        echo ""

        log_error "SLURM Compute Node test framework failed after ${duration}s"
        exit 1
    fi
}
...
COMMANDS:
    ...
    deploy-ansible      Deploy SLURM compute configuration via Ansible
    run-tests           Run SLURM compute test suite on deployed cluster
...
EXAMPLES:
    ...
    $0 run-test check-slurm-installation.sh   # Run specific test
...
case "$COMMAND" in
    ""|e2e|end-to-end|full)
        # Default: run complete end-to-end test
        main
        ;;
    start-cluster)
